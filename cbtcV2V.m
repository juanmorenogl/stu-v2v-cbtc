function [posicionActualizada,velocidadActualizada,aceleracionActualizada,limiteMax,contadorFrenados,frenadoEmergencia,distanciaTrenes,tiempoCBTC,incrementoTiempoSimulacion,posicionTrenPrecedenteActualizada,contadoraux] = cbtcV2V (matrizTren,matrizInterEstaciones,matrizCanal,KPIv2v,n,posicionTrenPrecedente,velocidadTrenPrecedente)
    ...
    % matrizCanal = matriz que contiene informacion sobre los
    % parámetros del canal de comunicaciones    
    % KPIv2v = matriz que contiene estadísticas relativas al tren
    ...
    % tiempoCBTC = tiempo que el tren ha usado CBTC-V2V
    ...
    retardoTrenTrenMax = abs(random('norm',matrizCanal(1,13),0.1));%Retardo maximo comunicacion tren-tren
    distanciaTrenes = KPIv2v(n,2);%Distancia que se ha calculado entre un tren y su precedente
    contadorFrenados = KPIv2v(n,4);%contador de accionamientos de frenado de emergencia
    tiempoCBTC = KPIv2v(n,6);%Tiempo de uso del sistema CBTC-V2V
    incrementoTiempoSimulacion = 0;
    posicionTrenPrecedenteActualizada = posicionTrenPrecedente;
    contadoraux = KPIv2v(n,13);
	...
	velocidadRetardo = [retardoTrenTrenMax/7 (retardoTrenTrenMax/7)*2 (retardoTrenTrenMax/7)*3 (retardoTrenTrenMax/7)*4 (retardoTrenTrenMax/7)*5 (retardoTrenTrenMax/7)*6 retardoTrenTrenMax];

    %Si el tren precedente esta parado, su velocidad será cero y ,por tanto,su posicion y velocidad seran las mismas que tenia anteriormente.No
    %habria que tener en cuenta los retardos de comunicacion para conocer su posicion exacta
    if velocidadTrenPrecedente > 0
        ...
        %Dependiendo del intervalo en el que se encuentre, se le asigna un retardo anteriormente definido
        ...
        if (velocidadRecibida >= 0) && (velocidadRecibida < 5) % 0 < V < 5 m/s
            ...
            retardoTotal = tiempoProcesado + velocidadRetardo(1);%Se suman los retardos de comunicacion y los de procesamiento
        ...
        elseif (velocidadRecibida >= 5) && (velocidadRecibida < 10)% 5 <= V < 10 m/s
            ...
            retardoTotal = tiempoProcesado + velocidadRetardo(2);%Se suman los retardos de comunicacion y los de procesamiento
            posicionaux = (velocidadRecibida).* retardoTotal;%Se calcula el espacio que ha avanzado el tren n+1 en el tiempo de retardo
            posicionTrenPrecedenteActualizada = posicionTrenPrecedente + posicionaux;%Se suma a la posicion que envia el tren precedente la posicion que ha avanzado el tren n+1 en el tiempo de comunicacion y procesado

        elseif (velocidadRecibida >= 10) && (velocidadRecibida < 15)% 10 <= V < 15 m/s

            retardoTotal = tiempoProcesado + velocidadRetardo(3);%Se suman los retardos de comunicacion y los de procesamiento
            posicionaux = (velocidadRecibida).* retardoTotal;%Se calcula el espacio que ha avanzado el tren n+1 en el tiempo de retardo
            posicionTrenPrecedenteActualizada = posicionTrenPrecedente + posicionaux;%Se suma a la posicion que envia el tren precedente la posicion que ha avanzado el tren n+1 en el tiempo de comunicacion y procesado

        elseif (velocidadRecibida >= 15) && (velocidadRecibida < 20)% 15 <= V < 20 m/s

            retardoTotal = tiempoProcesado + velocidadRetardo(4);%Se suman los retardos de comunicacion y los de procesamiento
            posicionaux = (velocidadRecibida).* retardoTotal;%Se calcula el espacio que ha avanzado el tren n+1 en el tiempo de retardo
            posicionTrenPrecedenteActualizada = posicionTrenPrecedente + posicionaux;%Se suma a la posicion que envia el tren precedente la posicion que ha avanzado el tren n+1 en el tiempo de comunicacion y procesado

        elseif (velocidadRecibida >= 20) && (velocidadRecibida < 25)% 20 <= V < 25 m/s

            retardoTotal = tiempoProcesado + velocidadRetardo(5);%Se suman los retardos de comunicacion y los de procesamiento
            posicionaux = (velocidadRecibida).* retardoTotal;%Se calcula el espacio que ha avanzado el tren n+1 en el tiempo de retardo
            posicionTrenPrecedenteActualizada = posicionTrenPrecedente + posicionaux;%Se suma a la posicion que envia el tren precedente la posicion que ha avanzado el tren n+1 en el tiempo de comunicacion y procesado

        elseif (velocidadRecibida >= 25) && (velocidadRecibida < 30)% 25 <= V < 30 m/s

            retardoTotal = tiempoProcesado + velocidadRetardo(6);%Se suman los retardos de comunicacion y los de procesamiento
            posicionaux = (velocidadRecibida).* retardoTotal;%Se calcula el espacio que ha avanzado el tren n+1 en el tiempo de retardo
            posicionTrenPrecedenteActualizada = posicionTrenPrecedente + posicionaux;%Se suma a la posicion que envia el tren precedente la posicion que ha avanzado el tren n+1 en el tiempo de comunicacion y procesado

        elseif velocidadRecibida >= 30%V > 30 m/s

            retardoTotal = tiempoProcesado + velocidadRetardo(7);%Se suman los retardos de comunicacion y los de procesamiento
            posicionaux = (velocidadRecibida).* retardoTotal;%Se calcula el espacio que ha avanzado el tren n+1 en el tiempo de retardo
            posicionTrenPrecedenteActualizada = posicionTrenPrecedente + posicionaux;%Se suma a la posicion que envia el tren precedente la posicion que ha avanzado el tren n+1 en el tiempo de comunicacion y procesado
    ...
    if frenadoEmergencia == 1
        ...
        if velocidadActualizada > 0
            ...
            posicionActualizada = posicionActualizada + ((tiempoProcesado + retardoTrenTrenMax/2).*(velocidadActualizada));%Se calcula la posicion que tiene el tren tras la comunicacion y el calculo del LMA
            tiempoFrenado = abs(velocidadActualizada/aceleracionActualizada);%Tiempo que el tren estará frenando
            distanciaFrenado = abs((((-1*aceleracionActualizada)*(tiempoFrenado.^2))/2) + velocidadActualizada*tiempoFrenado + random('norm',5,1));%distancia maxima que recorrera el tren cuando este frenando 
            ...
            if (posicionActualizada + (longitudTren/2)) >= LMA
                ...
                tiempoCBTC = tiempoCBTC + tiempoFrenado;%Se incrementa el tiempo que se usa CBTC de acuerdo al de comunicacion entre trenes,realizacion de calculos y el frenado
    ...
    tiempoCBTC = tiempoCBTC + tiempoProcesado/2 + retardoTrenTrenMax/2;%Se incrementa el tiempo que se usa CBTC de acuerdo al de realizacion de calculos y comunicacion con el tren delantero
    incrementoTiempoSimulacion = incrementoTiempoSimulacion + tiempoProcesado;%El tiempo de simulacion empleado sera el tiempo de procesado mas el retardo de comunicacion entre trenes
            
end%Fin funcion cbtcV2V